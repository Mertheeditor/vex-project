...
function normalizeBackendAudit(audit: Record<string, unknown>): SeoAuditResult {
  ...
  return {
    ...
    keyword_recommendations: pages.map((page) => ({
      ...
      recommendation: "Mevcut sayfa başlığı ve H1 üzerinden oluşturulmuş heuristic anahtar kelime yerleşim önerisi.",
      reason: "Bu MVP gerçek arama hacmi, trafik veya sıralama verisi üretmez.",
      source: "heuristic",
    })),
    content_recommendations: arrayOfRecords(audit.recommendations).map((item) => ({
      title: stringValue(item.title),
      recommendation: stringValue(item.detail),
      reason: "Deterministik SEO bulgularından üretildi.",
      source: "heuristic",
      page_url: "",
    })),
